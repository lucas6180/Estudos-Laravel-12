...
<div class=" flex flex-col gap-4 w-[screen] h-[950px] justify-center items-center">
    ...
    <div>
        <img id="imagemLivro" data-imagemPadrao="{{ asset('images/icon-livro.png') }}"
            src="{{ $livro ? asset($livro->imagem) : asset('images/icon-livro.png') }}"
            class="w-[150px] h-[150px] bg-cover">
    </div>
    ...
    <div class="flex justify-center flex-col w-4/6 gap-1">
        ...
        <textarea name="sinopse" rows="5" class="bg-slate-900 rounded-sm border-0 p-3" placeholder="Insira a Sinopse">{{ $livro->sinopse ?? '' }}</textarea>
    </div>
    <input required class="hidden" type="text" id="enderecoImg" name="imagem" value="{{ $livro->imagem ?? ''}}">
    <div class="flex justify-center flex-col w-4/6 gap-1">
        ...
        <select name="categoria_id" id="categoria" class="bg-slate-900 rounded-sm border-0 h-[48px] text-white">
            ...
            @foreach ($categorias as $categoria)
                <option value="{{ $categoria->id }}" @if ($livro) selected @endif>
                    {{ $categoria->tipo }}
                </option>
            @endforeach
        </select>
    ...
    <div class="flex justify-center flex-col w-4/6 gap-1">
        ...
        <input required placeholder="Insira o Autor do livro" type="text" name="autor"
            class="bg-slate-900 rounded-sm border-0 h-[48px]" value="{{ $livro->autor ?? '' }}">
    </div>
    <div class="flex justify-center flex-col w-4/6 gap-1">
        ...
        <input required placeholder="Insira a quantidade total" type="number" name="quantidade_total"
            class="bg-slate-900 rounded-sm border-0 h-[48px]" value="{{ $livro->quantidade_total ?? '' }}">
    </div>
    <div class="flex justify-center flex-col w-4/6 gap-1">
        ...
        <input required placeholder="Insira a quantidade disponivel" type="number" name="quantidade_disponivel"
            class="bg-slate-900 rounded-sm border-0 h-[48px]" value="{{ $livro->quantidade_disponivel ?? '' }}">
    </div>
    <button type="submit"
